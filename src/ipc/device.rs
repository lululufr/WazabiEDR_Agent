...
pub fn run_pump_loop(
    ...
    while !SHUTDOWN.load(Ordering::Acquire) {
        ...
        if spool.is_some() || detection.is_some() {
            ...
            //
            // The emission allow-list (crate::filter) only gates the spool/
            // upload path: detection is local and runs on every event so a
            // tuned upload filter never silently disables a Waza rule.
            if let Some(engine) = detection {
                match encode_kernel_event_and_log(payload) {
                    Ok((line, log)) => {
                        if let Some(spool) = spool {
                            if crate::filter::allows("kernel_callback", &log.event_type) {
                ...
                match encode_kernel_event(payload) {
                    ...
                    Ok(None) => {
                        // Filtré par l'allow-list — non-émission, pas d'erreur.
                    }
                    Err(e) => eprintln!("[agent] spool encode error: {}", e),
                }
            }
